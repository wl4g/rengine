...
import com.wl4g.rengine.eventbus.recorder.store.EhCacheEventStore;
...
public class EhcacheEventRecorder extends AbstractEventRecorder {
    ...
    public EhcacheEventRecorder(ClientEventBusConfig config) {
        super(config);
        this.paddingCache = new EhCacheEventStore(config.getRecorder().getEhcache(), "padding");
        this.completedCache = new EhCacheEventStore(config.getRecorder().getEhcache(), "completed");
    }
