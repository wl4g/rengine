...
import com.wl4g.rengine.eventbus.RabbitmqEventBusService;
 ...
 * @since v1.0.0
 */
@ConditionalOnClass(RabbitmqEventBusService.class)
public class RabbitmqEventBusAutoConfiguration {

    @Bean
    public RengineEventBusService<?> rabbitmqEventBusService(ClientEventBusConfig eventBusConfig, EventRecorder eventRecorder) {
        return new RabbitmqEventBusService(eventBusConfig, eventRecorder);
    }
