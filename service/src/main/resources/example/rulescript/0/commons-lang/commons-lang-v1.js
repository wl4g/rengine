/**
 * Commons Lang v3.0.0 | (c) 2022 ~ 2030 wl4g Foundation, Inc.
 * Copyright 2017-2050 <jameswong1376@gmail.com>, Inc. x
 * Licensed under Apache2.0 (https://github.com/wl4g/infra/blob/master/LICENSE)
 */

'use strict';

// Exposing the APIs to outside.
var CommonUtil = {
    printSafeWarn: function(title, msg){
    ...
    isEmpty: function(param){
        if(param == null || param == undefined || param == '')
            return true;
        else if(CommonUtil.isFunction(param))
            return false;
        else
    ...
    isAnyEmpty: function(params){
        for(var i = 0; i < arguments.length; i++){
            if(CommonUtil.isEmpty(arguments[i]))
                return true;
        }
        return false;
    },
    isAnyContains: function(params, search){
        if(!CommonUtil.isArray(CommonUtil.checkEmpty("Arguments", params))){
    ...
    clone: function(obj) { // 或使用JSON.parse(JSON.stringify(oldObj))实现深度拷贝，注意：Object.assign(oldObj,newObj)只能浅层拷贝
        if(!CommonUtil.isEmpty(obj)){
            ...
            if (typeof obj !== "object") {
                ...
                for (var i in obj) {
                    newobj[i] = typeof obj[i] === "object" ? CommonUtil.clone(obj[i]) : obj[i];
                }
            }
            return newobj;
        }
    ...
    Http: {
        ...
        request: function(options) {
            ...
            try {
                ...
                CommonUtil.checkEmpty("url", url);
                // Create XMLHttpRequest
                var _xhr = null;
                if (!_xhr) {
                    _xhr = CommonUtil.Http.createXMLHttpRequest();
                }
                // Init XMLHttpRequest
    ...
    PlatformType: (function() {
        var ua = ((typeof navigator) == "undefined") ? "unsupported" : navigator.userAgent.toLowerCase();
        var mua = {
            ...
            TouchDevice: ((typeof window != 'undefined') && 'ontouchstart' in window) || /touch/.test(ua), // Touch Device
    ...
    language: function() {
        return navigator && (navigator.language || navigator.browserLanguage || navigator.systemLanguage).toLowerCase();
    },
    isZhCN: function() {
        return CommonUtil.language().indexOf('zh') >= 0;
    },
    // Convertion paramMap to formData url
    toUrl: function(templateMap, paramMap) {
        CommonUtil.checkEmpty("templateMap", templateMap);
        CommonUtil.checkEmpty("paramMap", paramMap);
        var formData = "";
        paramMap.forEach((value, key) => {
            ...
            if(!CommonUtil.isEmpty(key)) {
                if(key.startsWith("{") && key.endsWith("}")) {
                    ...
                    paramName = CommonUtil.checkEmpty("templateMap." + realKey, templateMap[realKey]);
                }
            } else {
    ...
    Codec: {
        ...
        toHex: function(str) {
            return CommonUtil.Codec.base64ToHex(CommonUtil.Codec.encodeBase64(str));
        },
        ...
        base64ToHex: function(str) { // convert a base64 string to hex
            ...
            for (var i = 0; i < str.length; ++i) {
                ...
                if (k == 0) {
                    ret += CommonUtil.int2char(v >> 2);
                    slop = v & 3;
                    k = 1;
                }
                else if (k == 1) {
                    ret += CommonUtil.int2char((slop << 2) | (v >> 4));
                    slop = v & 0xf;
                    k = 2;
                }
                else if (k == 2) {
                    ret += CommonUtil.int2char(slop);
                    ret += CommonUtil.int2char(v >> 2);
                    slop = v & 3;
                    k = 3;
                }
                    ...
                    ret += CommonUtil.int2char((slop << 2) | (v >> 4));
                    ret += CommonUtil.int2char(v & 0xf);
                    k = 0;
                }
            }
            if (k == 1) {
                ret += CommonUtil.int2char(slop << 2);
            }
            return ret;
        }
    },
    Crc16CheckSum: {
        ...
        crc16Modbus : function (obj) {
            var buffer = Object.prototype.toString.call(obj) === '[object Array]' ? obj : CommonUtil.Crc16CheckSum.strToByte(obj);
            var ucCRCHi = (0xffff & 0xff00) >> 8;
            var ucCRCLo = 0xffff & 0x00ff;
            var iIndex;
            for (var i = 0; i < buffer.length; ++i) {
                iIndex = (ucCRCLo ^ buffer[i]) & 0x00ff;
                ucCRCLo = ucCRCHi ^ CommonUtil.Crc16CheckSum._auchCRCHi[iIndex];
                ucCRCHi = CommonUtil.Crc16CheckSum._auchCRCLo[iIndex];
            }
    ...
    isIp : function(ip) {
        return CommonUtil.isIpv4(ip) || CommonUtil.isIpv6(ip);
    }
};
