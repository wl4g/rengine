...
import static com.wl4g.infra.common.minio.S3Policy.Action.GetBucketPolicyAction;
...
public final class RengineMinioPolicyTool {
    ...
    public static final List<S3Policy.Action> DEFAULT_POLICY_ACTIONS = asList(GetBucketPolicyAction, GetBucketLocationAction,
    ...
    public static void main(String[] args) throws Exception {
        try {
            CommandLineFacade line = CommandLineTool.builder()
                    ...
                    .option("b", "bucket", DEFAULT_BUCKET, "Bucket name")
                    .option("n", "policyName", DEFAULT_POLICY_NAME, "Policy name.")
            ...
            if (isBlank(policyJson)) {
                policyJson = S3Policy.builder()
                        ...
                        .statement(singletonList(Statement.builder()
                                ...
                                .action(asList(GetBucketPolicyAction, GetBucketLocationAction, GetBucketPolicyStatusAction,
            ...
            createTenantBucket(endpoint, region, adminAccessKey, adminSecretKey, bucket);
    ...
    private static void createTenantPolicy(
            ...
            String policyName,
            String policyJson) throws InvalidKeyException, NoSuchAlgorithmException, InvalidCipherTextException, IOException {
        MinioAdminClient adminCilent = MinioAdminClient.builder()
                ...
                .build();
        adminCilent.addCannedPolicy(policyName, policyJson);
        out.println(" Created Policy: " + policyName + " => " + policyJson);
    ...
    private static void createTenantUser(
            ...
            String accessKey,
            String secretKey,
            String policyName) throws InvalidKeyException, NoSuchAlgorithmException, InvalidCipherTextException, IOException {
        MinioAdminClient adminCilent = MinioAdminClient.builder()
                .endpoint(endpoint)
                .region(region)
                .credentials(adminAccessKey, adminSecretKey)
        ...
        adminCilent.addUser(accessKey, Status.ENABLED, secretKey, policyName, emptyList());
    ...
    private static void assignTenantPolicyToUser(
        ...
        MinioAdminClient adminCilent = MinioAdminClient.builder()
                ...
                .httpClient(defaultHttpClient)
                .build();
        adminCilent.setPolicy(accessKey, false, policyName);
        out.println("Assigned Policy: " + policyName + " => " + accessKey);
    }
    ...
    private static void createTenantBucket(
        ...
        if (!client.bucketExists(BucketExistsArgs.builder().bucket(bucket).build())) {
            ...
            out.println(" Created Bucket: " + bucket);
        } else {
            out.println(" Already Bucket: " + bucket);
        }
    }
