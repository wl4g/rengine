...
public interface DictService {
    ...
    default List<Dict> findDicts(@Nullable DictType type, @Nullable String key, @Nullable String value) {
        return query(QueryDict.builder().type(type).key(key).value(value).pageNum(1).pageSize(Integer.MAX_VALUE).build())
                .getRecords();
    }

    PageHolder<Dict> query(@NotNull QueryDict model);

    SaveDictResult save(@NotNull SaveDict model);

    DeleteDictResult delete(@NotNull DeleteDict model);
