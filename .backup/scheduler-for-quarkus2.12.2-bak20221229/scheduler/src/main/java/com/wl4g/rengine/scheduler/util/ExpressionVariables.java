...
public class ExpressionVariables {
    ...
    public ExpressionVariables from(Map<String, String> variables) {
        Map<String, InternalVariable> parseVariables = safeMap(variables).entrySet()
                .stream()
                .collect(
                        toMap(e -> e.getKey(), e -> new InternalVariable(e.getKey(), e.getValue(), buildFunction(e.getValue()))));
    ...
    static Callable<String> buildFunction(String expression) {
        // TODO
        // TODO
        // TODO
        return () -> Qute.fmt(expression).render();
    }
