...
final class AnnotationTypeMappings {
    ...
    private boolean isMappable(AnnotationTypeMapping source, @Nullable Annotation metaAnnotation) {
        return (metaAnnotation != null && !this.filter.matches(metaAnnotation)
                && !AnnotationFilter.PLAIN.matches(source.getAnnotationType()) && !isAlreadyMapped(source, metaAnnotation));
    }
     ...
     *            the set of annotations that we have already visited; used to
     *            avoid infinite recursion for recursive annotations which some
     *            JVM languages support (such as Kotlin)
     * @return type mappings for the annotation type
     */
     ...
     *            the repeatable containers that may be used by the
     *            meta-annotations
     * @param annotationFilter
     *            the annotation filter used to limit which annotations are
     *            considered
     * @return type mappings for the annotation type
     */
    static AnnotationTypeMappings forAnnotationType(
    ...
    private static class Cache {
        ...
        /**
         * Get or create {@link AnnotationTypeMappings} for the specified
         * annotation type.
         * 
         * @param annotationType
         *            the annotation type
         * @param visitedAnnotationTypes
         *            the set of annotations that we have already visited; used
         *            to avoid infinite recursion for recursive annotations
         *            which some JVM languages support (such as Kotlin)
         * @return a new or existing {@link AnnotationTypeMappings} instance
         */
        AnnotationTypeMappings get(
                Class<? extends Annotation> annotationType,
                Set<Class<? extends Annotation>> visitedAnnotationTypes) {
