...
final class AttributeMethods {
    ...
    /**
     * Determine if values from the given annotation can be safely accessed
     * without causing any {@link TypeNotPresentException
    ...
    /**
     * Check if values from the given annotation can be safely accessed without
        ...
        for (int i = 0; i < size(); i++) {
            if (canThrowTypeNotPresentException(i)) {
                try {
                    ...
                    throw new IllegalStateException("Could not obtain annotation attribute value for " + get(i).getName()
                            + " declared on " + annotation.annotationType(), ex);
                }
            }
        }
    }
    ...
    private void assertAnnotation(Annotation annotation) {
        ...
        if (this.annotationType != null) {
            Assert2.isInstanceOf(this.annotationType, annotation);
        }
    }

    /**
     * Get the attribute with the specified name or {@code null} if no matching
     * attribute exists.
     * 
     * @param name
    ...
    /**
     * Get the index of the attribute with the specified name, or {@code -1} if
     * there is no attribute with the name.
     * 
     * @param name
     *            the name to find
    ...
    /**
     * Get the index of the specified attribute, or {@code -1} if the attribute
     ...
     * 
     * @return {@code true} if there is at least one attribute method with a
     *         default value
     */
    ...
    /**
     * Determine if at least one of the attribute methods is a nested
     * annotation.
     * 
     * @return {@code true} if there is at least one attribute method with a
