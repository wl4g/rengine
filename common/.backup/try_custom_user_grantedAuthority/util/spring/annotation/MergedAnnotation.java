...
public interface MergedAnnotation<A extends Annotation> {
     ...
     * meta-annotation has a distance of {@code 2}, etc. A
     * {@linkplain #missing() missing} annotation will always return a distance
     * of {@code -1}.
     * 
     * @return the annotation distance or {@code -1} if the annotation is
     *         missing
     */
    int getDistance();
     ...
     * {@linkplain #missing() missing} annotation will always return an
    ...
    /**
     * Get the source of the meta-annotation, or {@code null} if the annotation
     * is not {@linkplain #isMetaPresent() meta-present}.
     * <p>
    ...
    /**
     * Get the root annotation, i.e. the {@link #getDistance() distance}
     * {@code 0} annotation as directly declared on the source.
     * 
    ...
    /**
     * Determine if the specified attribute name has a default value when
     * compared to the annotation declaration.
     * 
     * @param attributeName
     *            the attribute name
    ...
    /**
     * Get the default attribute value from the annotation as specified in the
     ...
     * 
     * @param predicate
     *            a predicate used to filter attribute names
    ...
    /**
     * Create a new view of the annotation that exposes non-merged attribute
     * values.
     * <p>
     * Methods from this view will return attribute values with only alias
     * mirroring rules applied. Aliases to {@link #getMetaSource() meta-source}
     ...
     * 
     * @param adaptations
     *            the adaptations that should be applied to the annotation
     *            values
    ...
    /**
     * Create a new {@link Map} instance of the given type that contains all the
     ...
     * <p>
     * The result is synthesized using a JDK {@link java.lang.reflect.Proxy
     * Proxy} and as a result may incur a computational cost when first invoked.
     * <p>
     * If this merged annotation was created
     * {@linkplain #of(AnnotatedElement, Class, Map) from} a map of annotation
     * attributes or default attribute values, those attributes will always be
     * synthesized into an annotation instance.
     * <p>
     * If this merged annotation was created {@linkplain #from(Annotation) from}
     * an annotation instance, that annotation will be returned unmodified if it
     * is not <em>synthesizable</em>. An annotation is considered synthesizable
     * if it has not already been synthesized and one of the following is true.
     * <ul>
     * <li>The annotation declares attributes annotated with
     * {@link AliasFor @AliasFor}.</li>
     * <li>The annotation is a composed annotation that relies on
     * convention-based annotation attribute overrides in meta-annotations.</li>
     * <li>The annotation declares attributes that are annotations or arrays of
     * annotations that are themselves synthesizable.</li>
     * </ul>
     * 
     * @return a synthesized version of the annotation or the original
     *         annotation unmodified
     * @throws NoSuchElementException
    ...
    /**
     * Optionally create a type-safe synthesized version of this annotation
     * based on a condition predicate.
     * <p>
     * The result is synthesized using a JDK {@link java.lang.reflect.Proxy
     * Proxy} and as a result may incur a computational cost when first invoked.
     * <p>
     * Consult the documentation for {@link #synthesize()} for an explanation of
     * what is considered synthesizable.
     * 
     * @param condition
     *            the test to determine if the annotation can be synthesized
     * @return an optional containing the synthesized version of the annotation
     *         or an empty optional if the condition doesn't match
     * @throws NoSuchElementException
     *             on a missing annotation
     * @see MergedAnnotationPredicates
     */
     ...
     *            information and logging. It does not need to <em>actually</em>
     *            contain the specified annotations, and it will not be
     *            searched.
     * @param annotation
     *            the annotation to include
     ...
     *            the annotation type
     * @param attributes
     *            the annotation attributes or {@code null} if just default
     ...
     * 
     * @param classLoader
     *            the class loader used to resolve class attributes
     * @param source
     *            the source for the annotation. This source is used only for
     *            information and logging. It does not need to <em>actually</em>
     *            contain the specified annotations and it will not be searched.
     ...
     * {@link MergedAnnotation#asAnnotationAttributes(Adapt...)
    ...
    enum Adapt {
        ...
        /**
         * Adapt nested annotation or annotation arrays to maps rather than
         * synthesizing the values.
         */
        ANNOTATION_TO_MAP;
        ...
        /**
         * Factory method to create an {@link Adapt} array from a set of boolean
         * flags.
         * 
         * @param classToString
         *            if {@link Adapt#CLASS_TO_STRING} is included
         * @param annotationsToMap
         *            if {@link Adapt#ANNOTATION_TO_MAP} is included
