...
final class PackagesAnnotationFilter implements AnnotationFilter {
    ...
    PackagesAnnotationFilter(String... packages) {
        Assert2.notNull(packages, "Packages array must not be null");
        this.prefixes = new String[packages.length];
        for (int i = 0; i < packages.length; i++) {
            String pkg = packages[i];
            Assert2.hasText(pkg, "Packages array must not have empty elements");
            this.prefixes[i] = pkg + ".";
        }
    ...
    public String toString() {
        return "Packages annotation filter: " + StringUtils2.arrayToCommaDelimitedString(this.prefixes);
    }
