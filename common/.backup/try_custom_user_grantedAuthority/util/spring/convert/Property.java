 ...
 * {@code java.beans.PropertyDescriptor}. The {@code java.beans} package is not
 * available in a number of environments (e.g. Android, Java ME), so this is
...
public final class Property {
    ...
    /**
     * The object declaring this property, either directly or in a superclass
     * the object extends.
     */
    ...
    private String resolveName() {
        if (this.readMethod != null) {
            ...
            return StringUtils2.uncapitalize(this.readMethod.getName().substring(index));
            ...
            index += 3;
            return StringUtils2.uncapitalize(this.writeMethod.getName().substring(index));
        } else {
    ...
    private Field getField() {
        String name = getName();
        if (!StringUtils2.hasLength(name)) {
            return null;
        }
        Field field = null;
        Class<?> declaringClass = declaringClass();
        if (declaringClass != null) {
            field = ReflectionUtils2.findField(declaringClass, name);
            if (field == null) {
                // Same lenient fallback checking as in
                // CachedIntrospectionResults...
                field = ReflectionUtils2.findField(declaringClass, StringUtils2.uncapitalize(name));
                if (field == null) {
                    field = ReflectionUtils2.findField(declaringClass, StringUtils2.capitalize(name));
                }
            }
        }
        return field;
    ...
    public boolean equals(@Nullable Object other) {
        ...
        return (ObjectUtils2.nullSafeEquals(this.objectType, otherProperty.objectType)
                && ObjectUtils2.nullSafeEquals(this.name, otherProperty.name)
                && ObjectUtils2.nullSafeEquals(this.readMethod, otherProperty.readMethod)
                && ObjectUtils2.nullSafeEquals(this.writeMethod, otherProperty.writeMethod));
    }

    @Override
    public int hashCode() {
        return (ObjectUtils2.nullSafeHashCode(this.objectType) * 31 + ObjectUtils2.nullSafeHashCode(this.name));
    }
