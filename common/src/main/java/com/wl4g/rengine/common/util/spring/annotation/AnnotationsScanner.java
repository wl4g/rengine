...
import com.wl4g.rengine.common.util.spring.Ordered;
...
abstract class AnnotationsScanner {
     ...
     * 
     * @param context
     *            an optional context object that will be passed back to the
     *            processor
     * @param source
     *            the source element to scan
     * @param searchStrategy
     *            the search strategy to use
     * @param processor
     *            the processor that receives the annotations
        ...
        try {
            ...
            if (includeInterfaces) {
                for (Class<?> interfaceType : source.getInterfaces()) {
                    R interfacesResult = processClassHierarchy(context, aggregateIndex, interfaceType, processor, true,
                            includeEnclosing);
            ...
            if (superclass != Object.class && superclass != null) {
                R superclassResult = processClassHierarchy(context, aggregateIndex, superclass, processor, includeInterfaces,
                        includeEnclosing);
            ...
            if (includeEnclosing) {
                // Since merely attempting to load the enclosing class may
                // result in
                // automatic loading of sibling nested classes that in turn
                // results
                // in an exception such as NoClassDefFoundError, we wrap the
                // following
                // in its own dedicated try-catch block in order not to
                // preemptively
                // halt the annotation scanning process.
                try {
                    Class<?> enclosingClass = source.getEnclosingClass();
                    if (enclosingClass != null) {
                        R enclosingResult = processClassHierarchy(context, aggregateIndex, enclosingClass, processor,
        ...
        try {
            ...
            if (includeInterfaces) {
                for (Class<?> interfaceType : sourceClass.getInterfaces()) {
                    R interfacesResult = processMethodHierarchy(context, aggregateIndex, interfaceType, processor, rootMethod,
                            true);
                    if (interfacesResult != null) {
                        return interfacesResult;
                    }
            ...
            if (superclass != Object.class && superclass != null) {
                R superclassResult = processMethodHierarchy(context, aggregateIndex, superclass, processor, rootMethod,
                        includeInterfaces);
                if (superclassResult != null) {
                    return superclassResult;
                }
    ...
    private static <C> Method[] getBaseTypeMethods(C context, Class<?> baseType) {
        ...
        if (methods == null) {
            ...
            methods = isInterface ? baseType.getMethods() : ReflectionUtils2.getAllDeclaredMethods(baseType);
            int cleared = 0;
            for (int i = 0; i < methods.length; i++) {
                if ((!isInterface && Modifier.isPrivate(methods[i].getModifiers())) || hasPlainJavaAnnotationsOnly(methods[i])
    ...
    private static boolean isOverride(Method rootMethod, Method candidateMethod) {
        return (!Modifier.isPrivate(candidateMethod.getModifiers()) && candidateMethod.getName().equals(rootMethod.getName())
    ...
    private static <C, R> R processMethodAnnotations(
        ...
        if (bridgedMethod != source) {
            ...
            for (int i = 0; i < bridgedAnnotations.length; i++) {
                if (ObjectUtils2.containsElement(annotations, bridgedAnnotations[i])) {
