...
public abstract class MergedAnnotationPredicates {
    ...
    /**
     * Create a new {@link Predicate} that evaluates to {@code true} if the name
     * of the {@linkplain MergedAnnotation#getType() merged annotation type} is
     * contained in the specified array.
     * 
     * @param <A>
     *            the annotation type
     * @param typeNames
     *            the names that should be matched
    ...
    public static <A extends Annotation> Predicate<MergedAnnotation<? extends A>> typeIn(String... typeNames) {
        return annotation -> ObjectUtils2.containsElement(typeNames, annotation.getType().getName());
    }

    /**
     * Create a new {@link Predicate} that evaluates to {@code true} if the
     * {@linkplain MergedAnnotation#getType() merged annotation type} is
     * contained in the specified array.
     * 
     * @param <A>
     *            the annotation type
     * @param types
     *            the types that should be matched
     * @return a {@link Predicate} to test the annotation type
     */
    public static <A extends Annotation> Predicate<MergedAnnotation<? extends A>> typeIn(Class<?>... types) {
        return annotation -> ObjectUtils2.containsElement(types, annotation.getType());
    }
     ...
     *            the annotation type
     * @param types
     *            the type names or classes that should be matched
     * @return a {@link Predicate} to test the annotation type
     */
    ...
    /**
     * Create a new stateful, single use {@link Predicate} that matches only the
     * first run of an extracted value. For example,
     ...
     * same distance.
     * <p>
     * NOTE: This predicate only matches the first run. Once the extracted value
     * changes, the predicate always returns {@code false}. For example, if you
     * have a set of annotations with distances {@code [1, 1, 2, 1]} then only
     * the first two will match.
     * 
     * @param valueExtractor
     *            function used to extract the value to check
     ...
     * 
     * @param keyExtractor
     *            function used to extract the key used to test for uniqueness
    ...
    private static class FirstRunOfPredicate<A extends Annotation> implements Predicate<MergedAnnotation<A>> {
        ...
        FirstRunOfPredicate(Function<? super MergedAnnotation<A>, ?> valueExtractor) {
            Assert2.notNull(valueExtractor, "Value extractor must not be null");
        ...
        public boolean test(@Nullable MergedAnnotation<A> annotation) {
            ...
            return ObjectUtils2.nullSafeEquals(value, this.lastValue);
    ...
    private static class UniquePredicate<A extends Annotation, K> implements Predicate<MergedAnnotation<A>> {
        ...
        UniquePredicate(Function<? super MergedAnnotation<A>, K> keyExtractor) {
            Assert2.notNull(keyExtractor, "Key extractor must not be null");
