...
package com.wl4g.rengine.common.util.spring;
...
import com.wl4g.infra.common.reflect.ReflectionUtils2.MethodFilter;
 ...
 * <p>
 * See <a href=
 * "https://java.sun.com/docs/books/jls/third_edition/html/expressions.html#15.12.4.5">
...
public final class BridgeMethodResolver {
    ...
    public static Method findBridgedMethod(Method bridgeMethod) {
        ...
        if (bridgedMethod == null) {
            ...
            ReflectionUtils2.doWithMethods(bridgeMethod.getDeclaringClass(), candidateMethods::add, filter);
            if (!candidateMethods.isEmpty()) {
                bridgedMethod = candidateMethods.size() == 1 ? candidateMethods.get(0)
                        : searchCandidates(candidateMethods, bridgeMethod);
            }
            if (bridgedMethod == null) {
                // A bridge method was passed in but we couldn't find the
                // bridged method.
                // Let's proceed with the passed-in method and hope for the
                // best...
     ...
     * to filter for a set of possible matches.
     */
     ...
     * {@link Method} are equal after resolving all types against the
     * declaringType, otherwise returns {@code false}.
     */
    private static boolean isResolvedTypeMatch(Method genericMethod, Method candidateMethod, Class<?> declaringClass) {
        ...
        for (int i = 0; i < candidateParameters.length; i++) {
            ...
            if (!ClassUtils2.resolvePrimitiveIfNecessary(candidateParameter)
                    .equals(ClassUtils2.resolvePrimitiveIfNecessary(genericParameter.toClass()))) {
                return false;
            }
        }
        return true;
    ...
    private static Method findGenericDeclaration(Method bridgeMethod) {
        ...
        Class<?>[] interfaces = ClassUtils2.getAllInterfacesForClass(bridgeMethod.getDeclaringClass());
    ...
    /**
     * Compare the signatures of the bridge method and the method which it
     * bridges. If the parameter and return types are the same, it is a
     * 'visibility' bridge method introduced in Java 6 to fix
