...
import com.wl4g.infra.common.reflect.ResolvableType.MethodParameter;
import com.wl4g.rengine.common.util.spring.annotation.AnnotatedElementUtils;
...
public class TypeDescriptor implements Serializable {
    ...
    private static final Class<?>[] CACHED_COMMON_TYPES = { boolean.class, Boolean.class, byte.class, Byte.class, char.class,
            Character.class, double.class, Double.class, float.class, Float.class, int.class, Integer.class, long.class,
            Long.class, short.class, Short.class, String.class, Object.class };
    ...
    public TypeDescriptor(Property property) {
        Assert2.notNull(property, "Property must not be null");
     ...
     * normalize to object-based types and not work with primitive types
     * directly.
     */
    public Class<?> getObjectType() {
        return ClassUtils2.resolvePrimitiveIfNecessary(getType());
    }
    ...
    /**
     * Return the underlying source of the descriptor. Will return a
     * {@link Field}, {@link MethodParameter} or {@link Type} depending on how
     * the {@link TypeDescriptor} was constructed. This method is primarily to
     * provide access to additional type information or meta-data that
    ...
    /**
     * Narrows this {@link TypeDescriptor} by setting its type to the class of
     * the provided value.
     * <p>
     * If the value is {@code null}, no narrowing is performed and this
     * TypeDescriptor is returned unchanged.
     * <p>
     * Designed to be called by binding frameworks when they read property,
     * field, or method return values. Allows such frameworks to narrow a
     * TypeDescriptor built from a declared property, field, or method return
     * value type. For example, a field declared as {@code java.lang.Object}
     * would be narrowed to {@code java.util.HashMap} if it was set to a
     * {@code java.util.HashMap} value. The narrowed TypeDescriptor can then be
     * used to convert the HashMap to some other type. Annotation and nested
     * type context is preserved by the narrowed copy.
     * 
     * @param value
     *            the value to use for narrowing this type descriptor
     * @return this TypeDescriptor narrowed (returns a copy with its type
     *         updated to the class of the provided value)
     */
     ...
     * 
     * @param superType
     *            the supertype to cast to (can be {@code null})
    ...
    public TypeDescriptor upcast(@Nullable Class<?> superType) {
        ...
        Assert2.isAssignable(superType, getType());
    ...
    public String getName() {
        return ClassUtils2.getQualifiedName(getType());
    }
     ...
     * <p>
     * As of Spring Framework 4.2, this method supports arbitrary levels of
     * meta-annotations.
     * 
    ...
    public boolean hasAnnotation(Class<? extends Annotation> annotationType) {
        if (this.annotatedElement.isEmpty()) {
            // Shortcut: AnnotatedElementUtils would have to expect
            // AnnotatedElement.getAnnotations()
            // to return a copy of the array, whereas we can do it more
            // efficiently here.
            return false;
        }
    ...
    /**
     * Obtain the annotation of the specified {@code annotationType} that is on
     ...
     * @param annotationType
     *            the annotation type
     * @return the annotation, or {@code null} if no such annotation exists on
     *         this type descriptor
     */
    @Nullable
     ...
     * returns {@code true} because a String value can be assigned to a
     * CharSequence variable. On the other hand,
     * {@code valueOf(Number.class).isAssignableTo(valueOf(Integer.class))}
     * returns {@code false} because, while all Integers are Numbers, not all
     * Numbers are Integers.
     * <p>
     * For arrays, collections, and maps, element and key/value types are
     * checked if declared. For example, a List&lt;String&gt; field value is
     * assignable to a Collection&lt;CharSequence&gt; field, but
     * List&lt;Number&gt; is not assignable to List&lt;Integer&gt;.
     * 
     * @return {@code true} if this type is assignable to the type represented
     *         by the provided type descriptor
     * @see #getObjectType()
     */
    ...
    /**
     * If this type is an array, returns the array's component type. If this
     * type is a {@code Stream}, returns the stream's component type. If this
     * type is a {@link Collection} and it is parameterized, returns the
     * Collection's element type. If the Collection is not parameterized,
     * returns {@code null} indicating the element type is not declared.
     * 
     * @return the array component type or Collection element type, or
     *         {@code null} if this type is not an array type or a
     *         {@code java.util.Collection} or if its element type is not
     *         parameterized
    ...
    /**
     * If this type is a {@link Collection} or an array, creates an element
     * TypeDescriptor from the provided collection or array element.
     * <p>
     * Narrows the {@link #getElementTypeDescriptor() elementType} property to
     * the class of the provided collection or array element. For example, if
     * this describes a {@code java.util.List<java.lang.Number>} and the element
     * argument is a {@code java.lang.Integer}, the returned TypeDescriptor will
     * be {@code java.lang.Integer}. If this describes a
     * {@code java.util.List<?>} and the element argument is a
     ...
     * @return an element type descriptor, narrowed to the type of the provided
     *         element
    ...
    /**
     * If this type is a {@link Map} and its key type is parameterized, returns
     * the map's key type. If the Map's key type is not parameterized, returns
     * {@code null} indicating the key type is not declared.
     * 
     * @return the Map key type, or {@code null} if this type is a Map but its
     *         key type is not parameterized
     * @throws IllegalStateException
     *             if this type is not a {@code java.util.Map}
     */
    @Nullable
    public TypeDescriptor getMapKeyTypeDescriptor() {
        Assert2.state(isMap(), "Not a [java.util.Map]");
     ...
     * <p>
     * Narrows the {@link #getMapValueTypeDescriptor() mapValueType} property to
     * the class of the provided map value. For example, if this describes a
    ...
    public boolean equals(@Nullable Object other) {
        ...
        if (isCollection() || isArray()) {
            return ObjectUtils2.nullSafeEquals(getElementTypeDescriptor(), otherDesc.getElementTypeDescriptor());
        } else if (isMap()) {
            return (ObjectUtils2.nullSafeEquals(getMapKeyTypeDescriptor(), otherDesc.getMapKeyTypeDescriptor())
                    && ObjectUtils2.nullSafeEquals(getMapValueTypeDescriptor(), otherDesc.getMapValueTypeDescriptor()));
        } else {
            return true;
        }
    }
    ...
    private boolean annotationEquals(Annotation ann, Annotation otherAnn) {
        // Annotation.equals is reflective and pretty slow, so let's check
        // identity and proxy type first.
     ...
     * {@link #valueOf(Class)} to build a TypeDescriptor from the object's
     * class.
     * 
     * @param source
     *            the source object
     ...
     * descriptors from source objects, as it handles the {@code null} object
     * case.
     * 
     * @param type
     *            the class (may be {@code null} to indicate
     *            {@code Object.class})
     ...
     * method. The method call to construct such a {@code TypeDescriptor} would
     * look something like:
     ...
     *            a descriptor for the collection's element type, used to
    ...
    public static TypeDescriptor collection(Class<?> collectionType, @Nullable TypeDescriptor elementTypeDescriptor) {
        Assert2.notNull(collectionType, "Collection type must not be null");
     ...
     * <p>
     * For example, a Map&lt;String, String&gt; could be converted to a
     * Map&lt;Id, EmailAddress&gt; by converting to a targetType built with this
     * method: The method call to construct such a TypeDescriptor would look
     * something like:
     * 
     * <pre class="code">
    ...
    public static TypeDescriptor map(
        ...
        Assert2.notNull(mapType, "Map type must not be null");
     ...
     * <pre class="code">
     * TypeDescriptor
     *         .array(TypeDescriptor.map(Map.class, TypeDescriptor.value(String.class), TypeDescriptor.value(String.class)));
     * </pre>
     * 
     * @param elementTypeDescriptor
     *            the {@link TypeDescriptor} of the array element or
     *            {@code null}
     ...
     * level is 1, the nested type descriptor will be String, derived from the
     * map value.
     * <p>
     * If the methodParameter is a {@code List<Map<Integer, String>>} and the
     * nesting level is 2, the nested type descriptor will be String, derived
     * from the map value.
     * <p>
     * Returns {@code null} if a nested type cannot be obtained because it was
     * not declared. For example, if the method parameter is a {@code List<?>},
     * the nested type descriptor returned will be {@code null}.
     * 
     * @param methodParameter
     *            the method parameter with a nestingLevel of 1
     * @param nestingLevel
     *            the nesting level of the collection/array element or map
     *            key/value declaration within the method parameter
     * @return the nested type descriptor at the specified nesting level, or
     *         {@code null} if it could not be obtained
     * @throws IllegalArgumentException
     *             if the nesting level of the input {@link MethodParameter}
     *             argument is not 1, or if the types up to the specified
     *             nesting level are not of collection, array, or map types
     */
    @Nullable
     ...
     * <p>
     * For example, if the field is a {@code List<String>} and the nesting level
     * is 1, the nested type descriptor will be {@code String.class}.
     * <p>
     * If the field is a {@code List<List<String>>} and the nesting level is 2,
     * the nested type descriptor will also be a {@code String.class}.
     * <p>
     * If the field is a {@code Map<Integer, String>} and the nesting level is
     * 1, the nested type descriptor will be String, derived from the map value.
     * <p>
     * If the field is a {@code List<Map<Integer, String>>} and the nesting
     * level is 2, the nested type descriptor will be String, derived from the
     * map value.
     * <p>
     * Returns {@code null} if a nested type cannot be obtained because it was
     * not declared. For example, if the field is a {@code List<?>}, the nested
     * type descriptor returned will be {@code null}.
     * 
     * @param field
     *            the field
     * @param nestingLevel
     ...
     *         {@code null} if it could not be obtained
     * @throws IllegalArgumentException
     *             if the types up to the specified nesting level are not of
     ...
     * <p>
     * If the property is a {@code List<List<String>>} and the nesting level is
     ...
     * is 1, the nested type descriptor will be String, derived from the map
     * value.
     * <p>
     ...
     * <p>
     * Returns {@code null} if a nested type cannot be obtained because it was
     * not declared. For example, if the property is a {@code List<?>}, the
     ...
     * @param nestingLevel
     *            the nesting level of the collection/array element or map
     *            key/value declaration within the property
     * @return the nested type descriptor at the specified nesting level, or
     *         {@code null} if it could not be obtained
     * @throws IllegalArgumentException
     *             if the types up to the specified nesting level are not of
    ...
    private static TypeDescriptor nested(TypeDescriptor typeDescriptor, int nestingLevel) {
        ...
        for (int i = 0; i < nestingLevel; i++) {
            if (Object.class == nested.getType()) {
                // Could be a collection type but we don't know about its
                // element type,
                // so let's just assume there is an element type of type
                // Object...
            } else {
    ...
    private class AnnotatedElementAdapter implements AnnotatedElement, Serializable {
        ...
        public boolean isEmpty() {
            return ObjectUtils2.isEmpty(this.annotations);
        }
