...
final class SynthesizedMergedAnnotationInvocationHandler<A extends Annotation> implements InvocationHandler {
    ...
    private SynthesizedMergedAnnotationInvocationHandler(MergedAnnotation<A> annotation, Class<A> type) {
        Assert2.notNull(annotation, "MergedAnnotation must not be null");
        Assert2.notNull(type, "Type must not be null");
        Assert2.isTrue(type.isAnnotation(), "Type must be an annotation");
    ...
    public Object invoke(Object proxy, Method method, Object[] args) {
        if (ReflectionUtils2.isEqualsMethod(method)) {
            return annotationEquals(args[0]);
        }
        if (ReflectionUtils2.isHashCodeMethod(method)) {
            return annotationHashCode();
        }
        if (ReflectionUtils2.isToStringMethod(method)) {
    ...
    private boolean annotationEquals(Object other) {
        ...
        for (int i = 0; i < this.attributes.size(); i++) {
            ...
            Object otherValue = ReflectionUtils2.invokeMethod(attribute, other);
            if (!ObjectUtils2.nullSafeEquals(thisValue, otherValue)) {
                return false;
            }
        }
        return true;
     ...
     *
     * <ul>
     * <li>non-ASCII, non-visible, and non-printable characters within a
     * character or String literal are not escaped.</li>
     * <li>formatting for float and double values does not take into account
     * whether a value is not a number (NaN) or infinite.</li>
     * </ul>
     * 
     * @param value
    ...
    private Object getAttributeValue(Method method) {
        Object value = this.valueCache.computeIfAbsent(method.getName(), attributeName -> {
            Class<?> type = ClassUtils2.resolvePrimitiveIfNecessary(method.getReturnType());
        ...
        // Clone non-empty arrays so that users cannot alter the contents of
        // values in our cache.
