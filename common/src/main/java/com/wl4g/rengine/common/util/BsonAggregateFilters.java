...
public abstract class BsonAggregateFilters {
    ...
    public static final BsonArray USER_ROLE_MENU_LOOKUP_FILTERS = BsonArray.parse("["
            //+ "    { $match: { \"username\": { $in: [\"root\", \"huanglq1\"] } } },"
            + "{ $match: { \"enable\": { $eq: " + BaseBean.ENABLED + " } } },"
            + "{ $match: { \"delFlag\": { $eq: " + BaseBean.DEL_FLAG_NORMAL + " } } },"
            + "{ $project: { \"_class\": 0, \"delFlag\": 0, \"userRoles\": 0 } },"
            + "{ $lookup: {"
            + "    from: \"" + SYS_USER_ROLES.getName() + "\","
            ...
            + "                            }"
            + "                        },"
            + "                        { $replaceRoot: { newRoot: { $mergeObjects: [ { $arrayElemAt: [ \"$_menus\", 0 ] }, \"$$ROOT\" ] } } },"
            + "                        { $project: { \"_class\": 0, \"delFlag\": 0, \"menuId\": 0, \"roleId\": 0, \"_menus\": 0 } },"
            + "                    ],"
                                   // 原本为 menuRoles, 因为里层的 _menus 提取出来替换了这一层, 因此这一层就是 menus
            + "                    as: \"menus\""
            + "                    }"
            + "                },"
            + "            ],"
            ...
            + "        },"
                       // 将子节点menus提取合并到上一层:https://www.mongodb.com/docs/manual/reference/operator/aggregation/mergeObjects/#-mergeobjects
            + "        { $replaceRoot: { newRoot: { $mergeObjects: [ { $arrayElemAt: [ \"$_roles\", 0 ] }, \"$$ROOT\" ] } } },"
            + "        { $project: { \"_class\": 0, \"delFlag\": 0, \"userId\": 0, \"roleId\": 0, \"users\": 0, \"roles\": 0, \"_roles\": 0, \"userRoles\": 0, \"menuRoles\": 0 } },"
            + "    ],"
                   // 原本为 userRoles, 因为里层的 _roles 提取出来替换了这一层, 因此这一层就是 roles
            + "    as: \"roles\""
            + "    }"
            + "}"
            + "]");
