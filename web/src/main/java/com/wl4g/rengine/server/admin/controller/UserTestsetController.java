...
/**
 * {@link UserTestsetController}
 * 
 * @author James Wong
 * @version 2022-08-28
 * @since v3.0.0
 */
@Tag(name = "UserTestsetAPI", description = "The user custom testing dataset management API")
@Slf4j
@RestController
@RequestMapping("/admin/testset")
public class UserTestsetController {

    private @Autowired UserTestsetService testsestService;

    // @SecurityRequirement(name = "default_oauth")
    @Operation(summary = "Preparing to upload the user tests file.")
    ...
    public RespBase<UploadApplyResult> apply(HttpServletRequest request, UploadApply model) {
        ...
        resp.setData(testsestService.apply(model));
        return resp;
    }
