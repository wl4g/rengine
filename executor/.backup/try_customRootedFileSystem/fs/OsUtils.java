...
public final class OsUtils {
     ...
     * {@link #getCurrentUser()}. If not set then &quot;user.name&quot; system
     * property is used
     */
    ...
    /**
     * Property that can be used to override the reported value from
     * {@link #getJavaVersion()}. If not set then &quot;java.version&quot;
    ...
    /**
     * Property that can be used to override the reported value from
     * {@link #isWin32()}. If not set then &quot;os.name&quot; system property
     * is used
     */
    ...
    /**
     * Property that can be used to override the reported value from
     * {@link #isAndroid()}. If not set then
     * {@link #ANDROID_DETECTION_PROPERTIES} are used to determine its value.
     * Otherwise, it must contain the string &quot;android&quot;
     * (case-insensitive)
     *
    ...
    /**
     * Property that can be used to override the reported value from
     * {@link #isDalvikMachine()}. If not set then
     * {@link #DALVIK_DETECTION_PROPERTIES} are used to determine its value.
     * Otherwise, it must contain the string &quot;dalvik&quot;
     * (case-insensitive)
     */
    ...
    public static final List<String> LINUX_COMMAND = Collections
            .unmodifiableList(Arrays.asList(LINUX_SHELL_COMMAND_NAME, "-i", "-l"));
    public static final List<String> WINDOWS_COMMAND = Collections
            .unmodifiableList(Collections.singletonList(WINDOWS_SHELL_COMMAND_NAME));

    /**
     * System properties consulted in order to detect {@link #isAndroid()
     * Android O/S}.
     *
     * @see <A HREF=
     *      "https://developer.android.com/reference/java/lang/System#getProperties()">Android
     *      Developer</A>
     */
    public static final List<String> ANDROID_DETECTION_PROPERTIES = Collections.unmodifiableList(
            Arrays.asList("java.vendor", "java.specification.vendor", "java.vm.vendor", "java.vm.specification.vendor"));

    public static final Predicate<String> ANDROID_PROPERTY_VALUE_MATCHER = v -> GenericUtils.trimToEmpty(v)
            .toLowerCase()
            .contains("android");

    /**
     * System properties consulted in order to detect {@link #isDalvikMachine()
     * Dalvik machine}.
     *
     * @see <A HREF=
     *      "https://developer.android.com/reference/java/lang/System#getProperties()">Android
     *      Developer</A>
     */
    public static final List<String> DALVIK_DETECTION_PROPERTIES = Collections
            .unmodifiableList(Arrays.asList("java.specification.name", "java.vm.name", "java.vm.specification.name"));

    public static final Predicate<String> DALVIK_PROPERTY_VALUE_MATCHER = v -> GenericUtils.trimToEmpty(v)
            .toLowerCase()
            .contains("dalvik");
    ...
    /**
     * @return {@code true} if currently running on Android. <U>Note:</U>
     *         {@link #isUNIX()} is also probably {@code true} as well, so
     *         special care must be taken in code that consults these values
     * @see #ANDROID_DETECTION_PROPERTIES
     * @see #ANDROID_MODE_OVERRIDE_PROP
     * @see #ANDROID_PROPERTY_VALUE_MATCHER
     */
    public static boolean isAndroid() {
        return resolveAndroidSettingFlag(ANDROID_HOLDER, ANDROID_MODE_OVERRIDE_PROP, ANDROID_DETECTION_PROPERTIES,
     ...
     *
     * @param value
     *            Value to set if {@code null} then value is auto-detected
     */
    ...
    /**
     * @return {@code true} if currently running on a Dalvik machine.
     *         <U>Note:</U> {@link #isUNIX()} and/or {@link #isAndroid()} are
     *         also probably {@code true} as well, so special care must be taken
     *         in code that consults these values
    ...
    public static boolean isDalvikMachine() {
        return resolveAndroidSettingFlag(DALVIK_HOLDER, DALVIK_MACHINE_OVERRIDE_PROP, DALVIK_DETECTION_PROPERTIES,
                DALVIK_PROPERTY_VALUE_MATCHER);
    }

    /**
     * Override the value returned by {@link #isDalvikMachine()}
     * programmatically
     *
     * @param value
     *            Value to set if {@code null} then value is auto-detected
    ...
    /**
     * @return true if the host is a UNIX system (and not Windows). <U>Note:</U>
     *         this does <B>not</B> preclude {@link #isAndroid()} or
     *         {@link #isDalvikMachine()} from being {@code true} as well.
     */
     ...
     *
     * @param os
     *            The value to set - if {@code null} then O/S type is
     *            auto-detected
     * @see #isWin32()
     * @see #isOSX()
     * @see #isUNIX()
    ...
    private static boolean resolveAndroidSettingFlag(
            AtomicReference<Boolean> flagHolder,
            String overrideProp,
            Collection<String> detectionProps,
            Predicate<? super String> detector) {
    ...
    /**
     * @return The (C)urrent (W)orking (D)irectory {@link Path} - {@code null}
     *         if cannot resolve it. Resolution occurs as follows:
     *         <UL>
     *         <LI>Consult any currently registered
     *         {@link #setCurrentWorkingDirectoryResolver(Supplier)
     *         resolver}.</LI>
     *
     *         <LI>If no resolver registered, then &quot;user.dir&quot; system
     *         property is consulted.</LI>
     *         </UL>
     ...
     * {@link #getCurrentWorkingDirectory()} method
     *
     * @param cwdProvider
     *            The {@link Supplier} of the (C)urrent (W)orking (D)irectory
     *            {@link Path} - if {@code null} then &quot;user.dir&quot;
     *            system property is consulted
     */
     ...
     *
     * @param user
     *            The original username - ignored if {@code null}/empty
     ...
     *
     * @param group
     *            The original group name - used if not {@code null}/empty
     * @param user
     *            The owner name - sometimes it contains a group name
     ...
     *
     * @param username
     *            The username to set - if {@code null} then
     *            {@link #CURRENT_USER_OVERRIDE_PROP} will be consulted
     */
     ...
     * {@link #JAVA_VERSION_OVERRIDE_PROP}. If not set, then
     ...
     *
     * @param version
     *            The version - if {@code null} then it will be automatically
     *            resolved
     */
