 ...
 */
public class GraalRScriptEngineTests {

    GraalRScriptEngine engine;

    @Before
    public void init() {
        this.engine = new GraalRScriptEngine();
    ...
    public void testSimpleRunR() throws Exception {
        try (ContextWrapper graalContext = engine.getGraalPolyglotManager()
            ...
            // @formatter:on
            graalContext.eval(Source.newBuilder("r", script, "test.r").build());
            final Value bindings = graalContext.getBindings("r");
