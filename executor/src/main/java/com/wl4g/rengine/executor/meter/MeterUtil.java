...
import com.wl4g.rengine.executor.meter.RengineExecutorMeterService.MetricsName;
...
public class MeterUtil {
    ...
    static RengineExecutorMeterService meterService;
    ...
    public static <T> T timer(
        ...
        try {
            timer = getMeterService().timer(metricsName.getName(), metricsName.getHelp(),
                    RengineExecutorMeterService.DEFAULT_PERCENTILES, MetricsTag.SDK_DS_TYPE, dsType.name(), MetricsTag.SDK_DS_NAME,
    ...
    public static <T> T timer(
        ...
        try {
            timer = getMeterService().timer(metricsName.getName(), metricsName.getHelp(),
                    RengineExecutorMeterService.DEFAULT_PERCENTILES, MetricsTag.SDK_NOTIFIER_TYPE, notifierType.name(),
                    MetricsTag.METHOD_NAME, methodName);
        } catch (Throwable e) {
    ...
    public static <T> T timer(
        ...
        try {
            timer = getMeterService().timer(metricsName.getName(), metricsName.getHelp(),
                    RengineExecutorMeterService.DEFAULT_PERCENTILES, MetricsTag.SDK_CLIENT_TYPE, sdkType.getSimpleName(),
                    MetricsTag.METHOD_NAME, methodName);
        } catch (Throwable e) {
            log.error(format("Unable to timer meter for metricsName: '%s', methodName: '%s'", metricsName, methodName), e);
    ...
    static RengineExecutorMeterService getMeterService() {
        if (isNull(meterService)) {
            synchronized (MeterUtil.class) {
                if (isNull(meterService)) {
                    meterService = CDI.current().select(RengineExecutorMeterService.class).get();
                }
            }
        }
        return meterService;
