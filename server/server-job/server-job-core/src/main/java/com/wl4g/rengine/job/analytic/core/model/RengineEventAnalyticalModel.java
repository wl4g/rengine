...
/**
 * {@link RengineEventAnalyticalModel}
 * 
...
@NoArgsConstructor
@AllArgsConstructor
public class RengineEventAnalyticalModel {
    ...
    /**
     * Construction instance from {@link RengineEventBase}
     * 
     * @param event
     * @return
     */
    public static RengineEventAnalyticalModel of(@NotNull RengineEventBase event) {
        RengineEventAnalyticalModel model = new RengineEventAnalyticalModel();
    ...
    public static final List<Field> ORDERED_FIELDS = unmodifiableList(
            findAllDeclaredFields(RengineEventAnalyticalModel.class, true).stream()
                    .filter(f -> needPersistent(f))
                    .collect(toList()));
