...
public class MinioClientManager implements ApplicationRunner {
    ...
    public Credentials createSTSCredentialsWithAssumeRole() throws NoSuchAlgorithmException {
        UserUploadAssumeConfig uploadConfig = config.getUserUpload();
        String applyRoleArn = format("arn:aws:s3:::%s/%s/*", config.getBucket(), uploadConfig.getObjectPrefix())
        ...
        return createSTSCredentialsWithAssumeRole(config.getEndpoint(), uploadConfig.getAssumeAccessKey(),
                uploadConfig.getAssumeSecretKey(), applyRoleArn, safeLongToInt(uploadConfig.getExpiredDuration().getSeconds()),
                null);
    }
