...
public class MinioAdminClient {
    ...
    private Response execute(Method method, Command command, Multimap<String, String> queryParamMap, byte[] body)
    ...
    public void addCannedPolicy(@Nonnull String name, @Nonnull String policy)
        ...
        try (Response response = execute(Method.PUT, Command.ADD_CANNED_POLICY, ImmutableMultimap.of("name", name),
     ...
     *            User/Group name.
     * @param isGroup
     *            Flag to denote userOrGroupName is a group name.
     * @param policyName
     *            Policy name.
     * @throws NoSuchAlgorithmException
     *             thrown to indicate missing of MD5 or SHA-256 digest library.
    ...
    public void setPolicy(@Nonnull String userOrGroupName, boolean isGroup, @Nonnull String policyName)
        ...
        try (Response response = execute(Method.PUT, Command.SET_USER_OR_GROUP_POLICY, ImmutableMultimap.of("userOrGroup",
                userOrGroupName, "isGroup", String.valueOf(isGroup), "policyName", policyName), null)) {
        }
    }
     ...
     *
     * @return Map of policies, keyed by their name, with their actual policy as
     *         their value.
     * @throws NoSuchAlgorithmException
     *             thrown to indicate missing of MD5 or SHA-256 digest library.
     * @throws InvalidKeyException
    ...
    public Map<String, String> listCannedPolicies() throws NoSuchAlgorithmException, InvalidKeyException, IOException {
        try (Response response = execute(Method.GET, Command.LIST_CANNED_POLICIES, null, null)) {
            ...
            OBJECT_MAPPER.<Map<String, JsonNode>> readValue(response.body().bytes(), mapType)
    ...
    public void removeCannedPolicy(@Nonnull String name) throws NoSuchAlgorithmException, InvalidKeyException, IOException {
        ...
        try (Response response = execute(Method.DELETE, Command.REMOVE_CANNED_POLICY, ImmutableMultimap.of("name", name), null)) {
        }
    }

    /**
     * Sets HTTP connect, write and read timeouts. A value of 0 means no
     * timeout, otherwise values must be between 1 and Integer.MAX_VALUE when
     ...
     *             thrown to indicate key management error.
     * @throws NoSuchAlgorithmException
     *             thrown to indicate missing of SSL library.
     */
