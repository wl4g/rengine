...
public class DynamicCepOperatorFactory<IN, OUT> extends AbstractStreamOperatorFactory<OUT>
        implements OneInputStreamOperatorFactory<IN, OUT>, CoordinatedOperatorFactory<OUT>, ProcessingTimeServiceAware {
    ...
    public DynamicCepOperatorFactory(final PatternProcessorDiscovererFactory<IN> discovererFactory,
            final TypeSerializer<IN> inputSerializer, final TimeBehaviour timeBehaviour) {
    ...
    public <T extends StreamOperator<OUT>> T createStreamOperator(StreamOperatorParameters<OUT> parameters) {
        ...
        final DynamicCepOperator<IN, ?, OUT> patternProcessorOperator = new DynamicCepOperator<>(inputSerializer,
                timeBehaviour == TimeBehaviour.ProcessingTime);
        patternProcessorOperator.setup(parameters.getContainingTask(), parameters.getStreamConfig(), parameters.getOutput());
        patternProcessorOperator.setProcessingTimeService(parameters.getProcessingTimeService());
        parameters.getOperatorEventDispatcher().registerEventHandler(operatorId, patternProcessorOperator);
