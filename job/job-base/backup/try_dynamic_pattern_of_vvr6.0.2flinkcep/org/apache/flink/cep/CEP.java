 ...
 *
 * <p>
 * Methods which transform a {@link DataStream} into a {@link PatternStream} to
 * do CEP.
 */
public class CEP {
    /**
    ...
    /**
     * Creates a {@link DataStream} containing the results of
     * {@link PatternProcessFunction} to fully matching event pattern
     * processors.
     *
     * @param input
     *            DataStream containing the input events
     * @param patternProcessorDiscovererFactory
     *            Pattern processor discoverer factory to create the
     *            {@link PatternProcessorDiscoverer} for the
     *            {@link PatternProcessor} discovery
     * @param timeBehaviour
     *            How the system determines time for time-dependent order and
     *            operations that depend on time.
     * @param outTypeInfo
     *            Explicit specification of output type.
     * @param <T>
     *            Type of the input events
     * @param <R>
     *            Type of the resulting elements
     * @return {@link DataStream} which contains the resulting elements from the
     *         pattern processor discoverer
     */
    public static <T, R> SingleOutputStreamOperator<R> dynamicPatterns(
        ...
        final DynamicCepOperatorFactory<T, R> operatorFactory = new DynamicCepOperatorFactory<>(patternProcessorDiscovererFactory,
                input.getType().createSerializer(input.getExecutionConfig()), timeBehaviour);
