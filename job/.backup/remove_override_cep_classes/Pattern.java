// Currently upgraded to apache flink 1.16, some feature has been officially
// incorporated, such as:
// https://github1s.com/apache/flink/blob/release-1.16/flink-libraries/flink-cep/src/main/java/org/apache/flink/cep/pattern/Pattern.java#L67
//
// @formatter:off
//
///*
...
// * {@link org.apache.flink.cep.nfa.compiler.NFACompiler} to create a
// * {@link NFA}.
// *
// * <pre>{@code
...
//
//    /**
//     * A quantifier for the pattern. By default set to
//     * {@link Quantifier#one(ConsumingStrategy)}.
//     */
//    private Quantifier quantifier = Quantifier.one(ConsumingStrategy.STRICT);
//
//    /**
//     * The condition an event has to satisfy to stop collecting events into
//     * looping state.
//     */
//    private IterativeCondition<F> untilCondition;
//
//    /**
//     * Applicable to a {@code times} pattern, and holds the number of times it
//     * has to appear.
//     */
//    private Times times;
//
...
//
//    public Pattern(final String name, final Pattern<T, ? extends T> previous, final ConsumingStrategy consumingStrategy,
...
//     * initial pattern of the new sequence. Furthermore, the base type of the
//     * event sequence is set.
//     *
//     * @param name
...
//     *            the {@link AfterMatchSkipStrategy.SkipStrategy} to use after
//     *            each match.
//     * @param <X>
//     *            Base type of the event pattern
//     * @return The first pattern of a pattern sequence
...
//
//    /**
//     * Adds a condition that has to be satisfied by an event in order to be
//     * considered a match. If another condition has already been set, the new
//     * one is going to be combined with the previous with a logical {@code AND}.
...
//     * Adds a condition that has to be satisfied by an event in order to be
//     * considered a match. If another condition has already been set, the new
//     * one is going to be combined with the previous with a logical {@code OR}.
//     * In other case, this is going to be the only condition.
//     *
//     * @param condition
//     *            The condition as an {@link IterativeCondition}.
...
//     * event has to be of the given subtype in order to be matched.
//     *
//     * @param subtypeClass
...
//     *            a condition an event has to satisfy to stop collecting events
//     *            into looping state
...
//     * completed in order to be considered valid. This interval corresponds to
//     * the maximum time gap between first and the last event.
//     *
//     * @param windowTime
...
//     * Defines the maximum time interval in which a matching pattern has to be
//     * completed in order to be considered valid. This interval corresponds to
//     * the maximum time gap between events.
//     *
//     * @param withinType
...
//     * strict temporal contiguity. This means that the whole pattern sequence
//     * matches only if an event which matches this pattern directly follows the
//     * preceding matching event. Thus, there cannot be any events in between two
//     * matching events.
//     *
//     * @param name
...
//
//    /**
//     * Appends a new pattern to the existing one. The new pattern enforces that
//     * there is no event matching this pattern right after the preceding matched
//     * event.
//     *
//     * @param name
//     *            Name of the new pattern
...
//     * pattern and the preceding matching event might be interleaved with other
...
//    /**
//     * Appends a new pattern to the existing one. The new pattern enforces that
//     * there is no event matching this pattern between the preceding pattern and
//     * succeeding this one.
//     *
//     * <p>
//     * <b>NOTE:</b> There has to be other pattern after this one.
//     *
//     * @param name
//     *            Name of the new pattern
//     * @return A new pattern which is appended to this one
...
//
//    /**
//     * Specifies that this pattern can occur {@code one or more} times. This
//     * means at least one and at most infinite number of events can be matched
//     * to this pattern.
//     *
//     * <p>
//     * If this quantifier is enabled for a pattern
//     * {@code A.oneOrMore().followedBy(B)} and a sequence of events
//     * {@code A1 A2 B} appears, this will generate patterns: {@code A1 B} and
//     * {@code A1 A2 B}. See also {@link #allowCombinations()}.
//     *
//     * @return The same pattern with a
//     *         {@link Quantifier#looping(ConsumingStrategy)} quantifier applied.
//     * @throws MalformedPatternException
//     *             if the quantifier is not applicable to this pattern.
//     */
...
//
//    /**
//     * Specifies that this pattern can occur {@code one or more} times and time
//     * interval corresponds to the maximum time gap between previous and current
//     * event for each times. This means at least one and at most infinite number
//     * of events can be matched to this pattern.
//     *
//     * <p>
//     * If this quantifier is enabled for a pattern
//     * {@code A.oneOrMore().followedBy(B)} and a sequence of events
...
//
//    /**
//     * Specifies that this pattern is greedy. This means as many events as
//     * possible will be matched to this pattern.
//     *
...
//     * time interval corresponds to the maximum time gap between previous and
//     * current event for each times.
//     *
//     * @param times
//     *            number of times matching event must appear
//     * @param windowTime
...
//
//    /**
//     * Specifies that the pattern can occur between from and to times with time
//     * interval corresponds to the maximum time gap between previous and current
//     * event for each times.
//     *
//     * @param from
//     *            number of times matching event must appear at least
//     * @param to
...
//
//    /**
//     * Specifies that this pattern can occur the specified times at least. This
//     * means at least the specified times and at most infinite number of events
//     * can be matched to this pattern.
//     *
//     * @return The same pattern with a
//     *         {@link Quantifier#looping(ConsumingStrategy)} quantifier applied.
//     * @throws MalformedPatternException
...
//     * interval corresponds to the maximum time gap between previous and current
//     * event for each times. This means at least the specified times and at most
//     * infinite number of events can be matched to this pattern.
//     *
//     * @param times
//     *            number of times at least matching event must appear
//     * @param windowTime
//     *            time of the matching window between times
//     * @return The same pattern with a
//     *         {@link Quantifier#looping(ConsumingStrategy)} quantifier applied.
...
//     * {@link Quantifier#times(ConsumingStrategy)} patterns, this option allows
//     * more flexibility to the matching events.
//     *
//     * <p>
//     * If {@code allowCombinations()} is not applied for a pattern {@code
//     * A.oneOrMore().followedBy(B)} and a sequence of events {@code A1 A2 B}
//     * appears, this will generate patterns: {@code A1 B} and {@code A1 A2 B}.
//     * If this method is applied, we will have {@code A1 B}, {@code A2 B} and
//     * {@code A1 A2 B}.
//     *
...
//
//    /**
//     * Works in conjunction with {@link Pattern#oneOrMore()} or
//     * {@link Pattern#times(int)}. Specifies that any not matching element
//     * breaks the loop.
//     *
//     * <p>
//     * E.g. a pattern like:
...
//     *     }
//     * }).oneOrMore().consecutive().followedBy("end1").where(new SimpleCondition<Event>() { @Override
//     *     public boolean filter(Event value) throws Exception {
...
//
//    /**
//     * Appends a new group pattern to the existing one. The new pattern enforces
//     * non-strict temporal contiguity. This means that a matching event of this
//     * pattern and the preceding matching event might be interleaved with other
//     * events which are ignored.
//     *
...
//                    "Already applied quantifier to this Pattern. " + "Current quantifier is: " + quantifier);
//        }
//    }
//
//    /**
//     * @return the pattern's {@link AfterMatchSkipStrategy.SkipStrategy} after
//     *         match.
//     */
...
//        return "Pattern{" + "name='" + name + '\'' + ", previous=" + previous + ", condition=" + condition + ", windowTimes="
//                + windowTimes + ", quantifier=" + quantifier + ", untilCondition=" + untilCondition + ", times=" + times
